...
class BottomTabBarInteractor {
  ...
  private func openCrowdfunding() -> Bool {
    if let viewController,
       let donationUrl = Settings.donateUrl(),
       viewController.openUrl(donationUrl, externally: true) {
      configureTabBar()
      return true
    }
    return false
...
extension BottomTabBarInteractor: BottomTabBarInteractorProtocol {
  ...
  func openHelp() {
    if Settings.canShowCrowdfundingPromo() && openCrowdfunding() {
