  ...
  func setupViews() {
    ...
    func setupDonation() {
      ...
      if Settings.isCrowdfundingEnabled() {
        donationView = DonationView(.crowdfunding, action: { [weak self] in
          self?.openUrl(donationUrl, externally: true)
          Settings.didShowCrowdfundingPromo()
        })
      } else {
        donationView = DonationView(.donate, action: { [weak self] in
          self?.openUrl(donationUrl, externally: true)
        })
      }
    }
